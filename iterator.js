...
module.exports.get = function (collection, startPoint, depth) {
    ...
    return {
        next: function(name){
            if(Object.keys(collection).indexOf(startPoint) === -1 || currentNumber >= numberFriends){
                return null;
            }
            currentNumber++;
    ...
    while(queue.length !== 0){
        ...
        for(var i = 0; i < currentFriends.length; i++){
            if(addedNames.indexOf(currentFriends[i]) === -1){
                queue.push({name: currentFriends[i], countHandshake: currentFriend.countHandShake + 1});
